...
import { delegateEvent, EventError, forceResolveEvent, resolveEvent } from '../../lib/services/events.ts';
...
/**
 * Hands it to the assistant coach.
 *
 * They pick at random from whatever is open, which is rarely what you would have chosen. That
 * is the trade: it costs you control rather than money, and it means there is always one button
 * that works no matter how broke or short-handed the club is.
 */
export async function delegateEventAction(
  _prev: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const user = await requireUser();
  ...
  try {
    const team = await myTeam(leagueId, user.id);
    const result = await delegateEvent({ eventId, teamId: team.id, actorUserId: user.id });
    refresh(leagueId);
    return { success: `Your assistant went with: ${outcome(result)}` };
  } catch (error) {
    return { error: toMessage(error) };
  }
}

/** Commissioner escape hatch, so a stuck league can always move on. */
export async function forceResolveEventAction(
  _prev: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const user = await requireUser();
  ...
  try {
    const result = await forceResolveEvent({ eventId, actorUserId: user.id });
    refresh(leagueId);
    return { success: `Forced through: ${outcome(result)}` };
  } catch (error) {
    return { error: toMessage(error) };
  }
}
